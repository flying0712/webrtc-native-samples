...
static bool is_supported(void) { return true; }
...
static bool nvenc_init(void *nvenc_data, void *encoder_config) {
  ...
  nv_enc->nvenc = new NvEncoderCuda(*(nv_enc->cuContextPtr), nv_enc->width,
                                    nv_enc->height, nv_enc->format);
  ...
  nv_enc->nvenc->CreateDefaultEncoderParams(&initializeParams, codecId,
  ...
  // encodeConfig.rcParams.averageBitRate = (static_cast<unsigned int>(5.0f *
  // initializeParams.encodeWidth * initializeParams.encodeHeight) / (1280 *
  // 720)) * 100000;
...
int nvenc_set_bitrate(void *nvenc_data, uint32_t bitrate_bps) {
  ...
  if (nv_enc->nvenc != nullptr) {
    ...
    reconfigureParams.reInitEncodeParams.encodeConfig->rcParams.vbvBufferSize =
        reconfigureParams.reInitEncodeParams.encodeConfig->rcParams
            .averageBitRate *
        reconfigureParams.reInitEncodeParams.frameRateDen /
        reconfigureParams.reInitEncodeParams.frameRateNum;
    reconfigureParams.reInitEncodeParams.encodeConfig->rcParams
        .vbvInitialDelay = reconfigureParams.reInitEncodeParams.encodeConfig
...
int nvenc_set_framerate(void *nvenc_data, uint32_t framerate) {
  ...
  if (enc->nvenc != nullptr) {
    ...
    //不改变frameRateDen（默认为1），仅改变framerate； framerate = frameRateNum
    /// frameRateDen
...
int nvenc_encode(void *nvenc_data, uint8_t *in_buf, uint8_t *out_buf,
  ...
  NvEncoderCuda::CopyToDeviceFrame(
      *(enc->cuContextPtr), in_buf, 0, (CUdeviceptr)encoderInputFrame->inputPtr,
      (int)encoderInputFrame->pitch, enc->nvenc->GetEncodeWidth(),
      enc->nvenc->GetEncodeHeight(), CU_MEMORYTYPE_HOST,
...
int nvenc_get_sequence_params(void *nvenc_data, uint8_t *outBuf,
...
static bool nvenc_test0(void) {
  ...
  try {
    ...
    NV_ENC_BUFFER_FORMAT eInputFormat = NV_ENC_BUFFER_FORMAT_NV12;
    int nWidth = 544, nHeight = 960;
    nv_enc->nvenc = new NvEncoderCuda(*(nv_enc->cuContextPtr), nWidth, nHeight,
                                      eInputFormat);
    ...
    initializeParams.encodeConfig = &encodeConfig;
    GUID codecId = NV_ENC_CODEC_H264_GUID;
    nv_enc->nvenc->CreateDefaultEncoderParams(
        &initializeParams, codecId, NV_ENC_PRESET_LOW_LATENCY_DEFAULT_GUID);
    ...
    do {
      ...
      if (nRead == nHostFrameSize) {
        const NvEncInputFrame *encoderInputFrame =
            nv_enc->nvenc->GetNextInputFrame();

        if (true) {
          NvEncoderCuda::CopyToDeviceFrame(
              *(nv_enc->cuContextPtr), pHostFrame.get(), 0,
              (CUdeviceptr)encoderInputFrame->inputPtr,
              (int)encoderInputFrame->pitch, nv_enc->nvenc->GetEncodeWidth(),
              nv_enc->nvenc->GetEncodeHeight(), CU_MEMORYTYPE_HOST,
              encoderInputFrame->bufferFormat, encoderInputFrame->chromaOffsets,
              encoderInputFrame->numChromaPlanes);
        }
      ...
      for (std::vector<uint8_t> &packet : vPacket) {
        ...
        fpOut.write(reinterpret_cast<char *>(packet.data()),
                    (int)packet.size());
        nFrame++;
      }
    ...
    do {
      ...
      if (nRead == nHostFrameSize) {
        ...
        if (i && i % 20 == 1) {
          // NV_ENC_RECONFIGURE_PARAMS reconfigureParams =
          // {NV_ENC_RECONFIGURE_PARAMS_VER};
          // memcpy(&reconfigureParams.reInitEncodeParams, &initializeParams,
          // sizeof(initializeParams)); NV_ENC_CONFIG reInitCodecConfig =
          // {NV_ENC_CONFIG_VER}; memcpy(&reInitCodecConfig,
          // initializeParams.encodeConfig, sizeof(reInitCodecConfig));
          ...
          reconfigureParams.reInitEncodeParams = {NV_ENC_INITIALIZE_PARAMS_VER};
          reconfigureParams.reInitEncodeParams.encodeConfig = &encodeConfig;
          nv_enc->nvenc->GetInitializeParams(
              &reconfigureParams.reInitEncodeParams);

          // if (i % 40 != 1)
          {
            reconfigureParams.reInitEncodeParams.encodeConfig->rcParams
                .averageBitRate = reconfigureParams.reInitEncodeParams
                                      .encodeConfig->rcParams.averageBitRate /
                                  2;
            // reconfigureParams.reInitEncodeParams.encodeConfig->rcParams.averageBitRate
            // = 20;
            reconfigureParams.reInitEncodeParams.encodeConfig->rcParams
                ...
                reconfigureParams.reInitEncodeParams.frameRateNum;
            reconfigureParams.reInitEncodeParams.encodeConfig->rcParams
                .vbvInitialDelay = reconfigureParams.reInitEncodeParams
                                       .encodeConfig->rcParams.vbvBufferSize;
          }
          nv_enc->nvenc->Reconfigure(&reconfigureParams);
        }
        ...
        if (false) {
          ...
          int frame_size = nv_c_encoder.encode(
              nv_enc, pHostFrame.get(), out_buffer.get(), max_buffer_size);
          if (frame_size > 0) {
            nFrame++;
        ...
        for (std::vector<uint8_t> &packet : vPacket) {
          std::cout << "frame: " << nFrame + 1 << std::endl;
          // streamer.Stream(packet.data(), (int)packet.size(), nFrame++);
          fpOut.write(reinterpret_cast<char *>(packet.data()),
                      (int)packet.size());
          nFrame++;
